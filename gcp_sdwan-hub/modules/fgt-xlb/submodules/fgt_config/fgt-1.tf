...
data "template_file" "fgt_vpn-config" {
  ...
  vars = {
    ...
    ike-version           = lookup(var.hub[count.index], "ike_version", "2")
    dpd-retryinterval     = lookup(var.hub[count.index], "dpd_retryinterval", "10")
    ...
    bgp-asn_spoke         = var.hub[count.index]["bgp_asn_spoke"]
